...
def get_system_info():
    ...
    system_info = {
        ...
        "Username": username,  # Add username to system info
...
def copy_wallpaper_to_script_folder(wallpaper_path, script_folder, username):
    wallpaper_copy_path = os.path.join(script_folder, f"original_wallpaper_{username}.jpg")
...
def create_overlay_image(info, wallpaper_path, image_path=None):
    username = info["Username"]
    if image_path is None:
        image_path = f"overlay_{username}.png"  # Use a user-specific filename
...
current_user = os.environ['USERNAME']  # Get the current user

# Check if the current wallpaper is already user-specific
if not current_wallpaper.endswith(f"overlay_{current_user}.png"):
    if is_wallpaper_color():
        wallpaper_copy_path = create_gradient_background(script_folder)
    else:
        wallpaper_copy_path = copy_wallpaper_to_script_folder(current_wallpaper, script_folder, current_user)
else:
    wallpaper_copy_path = os.path.join(script_folder, f"original_wallpaper_{current_user}.jpg")

while True:
    system_info = get_system_info()
    create_overlay_image(system_info, wallpaper_copy_path)
    set_wallpaper(f"overlay_{current_user}.png")  # Use user-specific overlay
